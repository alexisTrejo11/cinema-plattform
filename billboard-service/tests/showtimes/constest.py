...
from app.showtime.infrastructure.persistence.sqlalch_show_repository import SQLAlchemyShowtimeRepository as ShowtimeRepository
...
def create_seats():
    ...
    def _create_seats(num_seats: int, taken_seats_count: int = 0) -> List[Seats]:
        ...
        for i in range(num_seats):
            ...
            seats.append(Seats(seat_id=i + 1, seat_row=row, seat_number=number, is_taken=is_taken))
        return seats
...
def sample_showtime(create_seats) -> Showtime:
    ...
    total_seats = 100
    available_seats = 90
    seats_list = create_seats(total_seats, total_seats - available_seats) # 10 asientos tomados
    
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(hours=2)
...
def old_showtime(create_seats) -> Showtime:
    ...
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) - timedelta(days=5, hours=3)
...
def future_showtime_different_cinema(create_seats) -> Showtime:
    ...
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(days=1, hours=5)
...
def future_showtime_different_movie(create_seats) -> Showtime:
    ...
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(hours=4)
...
def showtime_with_no_available_seats(create_seats) -> Showtime:
    ...
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(hours=24)
...
def showtime_too_far_in_future(create_seats) -> Showtime:
    ...
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(days=35) # Más de 30 días
    end_time_utc = start_time_utc + timedelta(minutes=120)
...
def showtime_overlapping_with_another(create_seats) -> Showtime:
    ...
    start_time_utc = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(hours=2, minutes=30)
    end_time_utc = start_time_utc + timedelta(minutes=90) # Termina antes que sample_showtime
