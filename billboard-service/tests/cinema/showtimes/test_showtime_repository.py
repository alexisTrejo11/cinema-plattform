...
@pytest.mark.asyncio
async def test_save_and_get_by_id(showtime_repo: ShowtimeRepository, sample_showtime: Showtime):
    # Arrange
    ...
    assert abs((fetched_showtime.start_time - sample_showtime.start_time).total_seconds()) < 1
    assert fetched_showtime.price == sample_showtime.price
    
@pytest.mark.asyncio
async def test_update_showtime(showtime_repo: ShowtimeRepository, sample_showtime: Showtime, showtime_for_update: Dict[str, Any]):
    # Arrange
    saved_showtime = await showtime_repo.save(sample_showtime)
    ...
    fetched_showtime: Optional[Showtime] = await showtime_repo.get_by_id(updated_showtime.id)
...
@pytest.mark.asyncio
async def test_delete_showtime(showtime_repo: ShowtimeRepository, sample_showtime: Showtime):
    # Arrange
    saved_showtime = await showtime_repo.save(sample_showtime)
    assert saved_showtime.id
...
@pytest.mark.asyncio
async def test_list_all_showtimes(showtime_repo: ShowtimeRepository, sample_showtime: Showtime, old_showtime: Showtime):
    # Arrange
...
async def test_list_incoming_by_cinema(
    ...
    future_showtime_different_cinema.start_time = datetime.now(timezone.utc) + timedelta(hours=3)
    future_showtime_different_cinema = await showtime_repo.save(future_showtime_different_cinema)
    ...
    assert incoming_showtimes[0].start_time >= datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
    assert incoming_showtimes[0].start_time < (datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1))
...
async def test_list_by_theater_and_date_range(showtime_repo: ShowtimeRepository):
    ...
    showtime1 = Showtime(
        id=None, movie_id=1, cinema_id=1, theater_id=theater_id,seats=[],
        start_time=datetime(2025, 6, 15, 10, 0, tzinfo=timezone.utc),
        end_time=datetime(2025, 6, 15, 12, 0, tzinfo=timezone.utc),
        available_seats=50, total_seats=50, price=Decimal("10.0"), type=ShowtimeType.TRADITIONAL_2D, 
    ...
    showtime_before = Showtime(
        id=None, movie_id=2, cinema_id=1, theater_id=theater_id,seats=[],
    ...
    showtime_after = Showtime(
        id=None, movie_id=3, cinema_id=1, theater_id=theater_id,seats=[],
    ...
    showtime_other_theater = Showtime(
        id=None, movie_id=4, cinema_id=1, theater_id=11,seats=[],
    ...
    showtime_overlap = Showtime(
        id=None, movie_id=5, cinema_id=1, theater_id=theater_id,seats=[],
...
async def test_list_by_filters_group_by_movie(showtime_repo: ShowtimeRepository):
    ...
    showtime_m1_c1_future = Showtime(
        id=None, movie_id=10, cinema_id=1, theater_id=1,seats=[],
        ...
        available_seats=10, total_seats=10, price=Decimal("10.0"), type=ShowtimeType.TRADITIONAL_2D,
        language=ShowtimeLanguage.ORIGINAL_ENGLISH
    )
    ...
    showtime_m1_c2_future = Showtime(
        id=None, movie_id=10, cinema_id=2, theater_id=1,seats=[],
    ...
    showtime_m2_c1_future = Showtime(
        id=None, movie_id=20, cinema_id=1, theater_id=2,seats=[],
    ...
    showtime_m1_c1_past = Showtime(
        ...
        end_time=datetime.now(timezone.utc).replace(microsecond=0) - timedelta(days=1, hours=2),
        available_seats=10, total_seats=10, price=Decimal("10.0"), type=ShowtimeType.TRADITIONAL_2D,
        language=ShowtimeLanguage.ORIGINAL_ENGLISH
    )
    # CAPTURE THE SAVED OBJECT WITH ITS ID
    ...
    result_incoming = await showtime_repo.list_by_filters_group_by_movie(filters_incoming, page_data)
    
    assert len(result_incoming) == 2 # Movie 10 and Movie 20
    assert 10 in result_incoming
    assert 20 in result_incoming
    assert len(result_incoming[10]) == 2 # saved_showtime_m1_c1_future, saved_showtime_m1_c2_future
    ...
    filters_incoming_cinema_1 = MovieShowtimesFilters(incoming=True, movie_id=None, cinema_id_list=[1])
    result_incoming_cinema_1 = await showtime_repo.list_by_filters_group_by_movie(filters_incoming_cinema_1, page_data)
    ...
    # Assert specific IDs
    assert any(s.id == saved_showtime_m1_c1_future.id for s in result_incoming_cinema_1[10])
    assert any(s.id == saved_showtime_m2_c1_future.id for s in result_incoming_cinema_1[20])


    # --- Test 3: Incoming and movie_id ---
    filters_incoming_movie_10 = MovieShowtimesFilters(incoming=True, movie_id=10)
    result_incoming_movie_10 = await showtime_repo.list_by_filters_group_by_movie(filters_incoming_movie_10, page_data)
    ...
    assert len(result_incoming_movie_10[10]) == 2 # saved_showtime_m1_c1_future, saved_showtime_m1_c2_future

    # Assert specific IDs
    assert any(s.id == saved_showtime_m1_c1_future.id for s in result_incoming_movie_10[10])
    assert any(s.id == saved_showtime_m1_c2_future.id for s in result_incoming_movie_10[10])


    # --- Test 4: Combination of all filters ---
    filters_all_combined = MovieShowtimesFilters(incoming=True, cinema_id_list=[1], movie_id=10)
    result_all_combined = await showtime_repo.list_by_filters_group_by_movie(filters_all_combined, page_data)
    ...
    filters_no_filters = MovieShowtimesFilters(movie_id=None, incoming=False) # Explicitly setting incoming=False
    result_no_filters = await showtime_repo.list_by_filters_group_by_movie(filters_no_filters, page_data)
    ...
    all_retrieved_showtimes = [s for movie_showtimes in result_no_filters.values() for s in movie_showtimes]
    ...
    assert len(result_no_filters[20]) == 1 # Expecting only the future showtime for movie 20