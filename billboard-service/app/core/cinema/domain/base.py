...
class CinemaBase(BaseModel):
    ...
    name: str = Field(
        ..., max_length=255, min_length=3, description="Name of the cinema."
    )
    ...
    is_active: bool = Field(
        False, description="True if the cinema is currently operational."
    )
    description: str = Field("", description="A brief description of the cinema.")
    screens: int = Field(
        ..., ge=0, description="Number of screens/theaters in the cinema."
    )
    last_renovation: Optional[date] = Field(
        None, description="Date of the last major renovation."
    )

    type: CinemaType = Field(
        ..., description="Type of cinema (e.g., VIP, Traditional)."
    )
    status: CinemaStatus = Field(..., description="Current operational status.")
    amenities: CinemaAmenities = Field(
        ..., description="Details about amenities available."
    )
    ...
    created_at: datetime = Field(
        ..., description="Date and time the cinema was created."
    )
    updated_at: datetime = Field(
        ..., description="Date and time the cinema was last updated."
    )
    deleted_at: Optional[datetime] = Field(
        None, description="Date and time the cinema was deleted."
    )
