...
from app.core.showtime.application.dtos import (
    ShowtimeCreate,
    ShowtimeUpdate,
...
async def get_showtime(
    request: Request,
    showtime_id: int,
    use_case: GetShowtimeByIdUseCase = Depends(showtime_use_cases.get_showtime_by_id_use_case),
):
...
async def search_showtimes(
    ...
    use_case: SearchShowtimesUseCase = Depends(showtime_use_cases.search_showtimes_use_case),
) -> PaginatedShowtimeResponse:
...
async def draft_showtime(
    ...
    use_case: DraftShowtimeUseCase = Depends(showtime_use_cases.draft_showtime_use_case),
...
async def launch_showtime(
    showtime_id: int,
    request: Request,
    use_case: LaunchShowtimeUseCase = Depends(showtime_use_cases.launch_showtime_use_case),
    current_user: AuthUserContext = Depends(require_roles("admin", "manager")),
):
    showtime = await use_case.execute(showtime_id)
    return ShowtimeDetailResponse.model_validate(showtime)
...
async def cancel_showtime(
    showtime_id: int,
    request: Request,
    use_case: CancelShowtimeUseCase = Depends(showtime_use_cases.cancel_showtime_use_case),
    current_user: AuthUserContext = Depends(require_roles("admin", "manager")),
):
    showtime = await use_case.execute(showtime_id)
    return ShowtimeDetailResponse.model_validate(showtime)
...
async def restore_showtime(
    showtime_id: int,
    request: Request,
    use_case: RestoreShowtimeUseCase = Depends(showtime_use_cases.restore_showtime_use_case),
    current_user: AuthUserContext = Depends(require_roles("admin", "manager")),
):
    showtime = await use_case.execute(showtime_id)
    return ShowtimeDetailResponse.model_validate(showtime)
...
async def update_showtime(
    ...
    use_case: UpdateShowtimeUseCase = Depends(showtime_use_cases.update_showtime_use_case),
    current_user: AuthUserContext = Depends(require_roles("admin", "manager")),
):
...
async def delete_showtime(
    request: Request,
    showtime_id: int,
    use_case: DeleteShowtimeUseCase = Depends(showtime_use_cases.delete_showtime_use_case),
    current_user: AuthUserContext = Depends(require_roles("admin", "manager")),
):
