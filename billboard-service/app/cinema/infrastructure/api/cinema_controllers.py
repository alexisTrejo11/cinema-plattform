...
async def get_cinema_by_id(
    ...
    logger.info(f"GET cinema started | cinema_id:{cinema_id} | client:{request.client.host if request.client else None}")
    try:
        cinema = await use_case.execute(cinema_id)
        logger.info(f"GET cinema success | cinema_id:{cinema_id}")
        return cinema
    except Exception as e:
        logger.error(f"GET cinema failed | cinema_id:{cinema_id} | error:{str(e)}")
        raise
...
async def get_active_cinemas(
    ...
    try:
        ...
        logger.error(f"GET active cinemas failed | error:{str(e)}")
        raise
...
async def search_cinemas(
    ...
    logger.info(f"SEARCH cinemas started | offset:{offset} | limit:{limit} | filters:{filters.model_dump()} | client:{request.client.host if request.client else None}")
    filter_params = {}
    try:
        ...
        logger.error(f"SEARCH cinemas failed | error:{str(e)} | filters:{filter_params}")
        raise
...
async def create_cinema(
    ...
    logger.info(f"POST cinema started | name:{cinema.name} | client:{request.client.host if request.client else None}")
    try:
        created_cinema = await use_case.execute(cinema)
        logger.info(f"POST cinema success | cinema_id:{created_cinema.id} | name:{created_cinema.name}")
...
async def update_cinemas(
    ...
    logger.info(f"PUT cinema started | cinema_id:{cinema_id} | client:{request.client.host if request.client else None}")
    try:
        updated_cinema = await use_case.execute(cinema_id, cinema)
        logger.info(f"PUT cinema success | cinema_id:{cinema_id} | name:{updated_cinema.name}")
...
async def delete_cinema(
    ...
    logger.info(f"DELETE cinema started | cinema_id:{cinema_id} | client:{request.client.host if request.client else None}")
    try:
