...
class SQLAlchemyCinemaRepository(CinemaRepository):
    ...
    def _build_filters_param(self, filters: SearchCinemaFilters) -> List[Any]:
        ...
        if filters.name:
            conditions.app.d(CinemaModel.name.ilike(f"%{filters.name}%"))
        if filters.tax_number:
            conditions.app.d(CinemaModel.tax_number == filters.tax_number)
        if filters.is_active is not None:
            conditions.app.d(CinemaModel.is_active == filters.is_active)
        if filters.has_parking is not None:
            conditions.app.d(CinemaModel.has_parking == filters.has_parking)
        if filters.has_food_court is not None:
            conditions.app.d(CinemaModel.has_food_court == filters.has_food_court)

        if filters.type:
            conditions.app.d(CinemaModel.type == filters.type)
        if filters.status:
            conditions.app.d(CinemaModel.status == filters.status)
        if filters.region:
            conditions.app.d(CinemaModel.region == filters.region)

        if filters.min_screens is not None:
            conditions.app.d(CinemaModel.screens >= filters.min_screens)
        if filters.max_screens is not None:
            conditions.app.d(CinemaModel.screens <= filters.max_screens)

        if filters.renovated_after:
            conditions.app.d(CinemaModel.last_renovation >= filters.renovated_after)
        if filters.renovated_before:
            conditions.app.d(CinemaModel.last_renovation <= filters.renovated_before)

        if filters.latitude is not None and filters.longitude is not None:
            conditions.app.d(CinemaModel.latitude == filters.latitude)
            conditions.app.d(CinemaModel.longitude == filters.longitude)

        if filters.phone:
            conditions.app.d(CinemaModel.phone == filters.phone)
