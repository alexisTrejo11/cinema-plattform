...
from app.theater.application.theather_use_cases import GetTheaterByIdUseCase, ListTheatersUseCase, CreateTheaterUseCase, UpdateTheaterUseCase, DeleteTheaterUseCase, GetTheatersByCinemaUseCase
from app.theater.domain.theater import Theater
from .depdencies import get_theater_by_id_use_case, list_theaters_use_case, create_theater_use_case, update_theater_use_case, delete_theater_use_case, get_theaters_by_cinema_use_case
import logging
...
async def get_theater(
    ...
    logger.info(f"GET theater started | theater_id:{theater_id} | client:{request.client.host if request.client else None}")
    try:
...
async def list_theaters(
    ...
    logger.info(f"LIST theaters started | page:{page} | limit:{limit} | client:{request.client.host if request.client else None}")
    try:
        ...
        logger.error(f"LIST theaters failed | page:{page} | limit:{limit} | error:{str(e)}")
        raise
...
async def get_theaters_by_cinema(
    ...
    logger.info(f"GET theaters by cinema started | cinema_id:{cinema_id} | client:{request.client.host if request.client else None}")
    try:
        theaters = await use_case.execute(cinema_id)
        logger.info(f"GET theaters by cinema success | cinema_id:{cinema_id} | count:{len(theaters)}")
        return theaters
    except Exception as e:
        logger.error(f"GET theaters by cinema failed | cinema_id:{cinema_id} | error:{str(e)}")
        raise
...
async def create_theater(
    ...
    logger.info(f"POST theater started | name:{new_theater.name} | client:{request.client.host if request.client else None}")
    try:
        theater = await use_case.execute(new_theater)
        logger.info(f"POST theater success | theater_id:{theater.id} | name:{theater.name}")
        return theater
    except Exception as e:
...
async def update_theater(
    ...
    logger.info(f"PUT theater started | theater_id:{theater_id} | client:{request.client.host if request.client else None}")
    try:
        theater = await use_case.execute(theater_id, update_theater)
        logger.info(f"PUT theater success | theater_id:{theater_id} | name:{theater.name}")
        return theater
    except Exception as e:
...
async def delete_theater(
    ...
    logger.info(f"DELETE theater started | theater_id:{theater_id} | client:{request.client.host if request.client else None}")
    try:
        await use_case.execute(theater_id)
        logger.info(f"DELETE theater success | theater_id:{theater_id}")
    except Exception as e:
        logger.error(f"DELETE theater failed | theater_id:{theater_id} | error:{str(e)}")
        raise