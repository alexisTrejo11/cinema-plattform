...
async def get_movie_by_id(
    ...
    logger.info(f"GET movie started | movie_id:{movie_id} | client:{request.client.host if request.client else None}")
    try:
...
async def get_movies_in_exhibition(
    ...
    logger.info(f"GET active movies started | client:{request.client.host if request.client else None}")
    try:
...
async def create_movies(
    ...
    logger.info(f"POST movie started | client:{request.client.host if request.client else None} | title:{movie.title}")
    try:
        created_movie = await use_case.execute(movie)
        logger.info(f"POST movie success | movie_id:{created_movie.id} | title:{created_movie.title}")
...
async def update_movie(
    ...
    logger.info(f"PUT movie started | movie_id:{movie_id} | client:{request.client.host if request.client else None}")
    try:
        updated_movie = await use_case.execute(movie_id, movie)
        logger.info(f"PUT movie success | movie_id:{movie_id} | new_title:{updated_movie.title}")
...
async def delete_movie(
    ...
    logger.info(f"DELETE movie started | movie_id:{movie_id} | client:{request.client.host if request.client else None}")
    try:
