...
async def handle_domain_exceptions(request: Request, exc: DomainException):
    ...
    error_reponse = ApiResponse.failure(
        ErrorResponse(code=exc.error_code, type="Domain", details=exc.details),
        exc.message,
    )
    return JSONResponse(status_code=exc.status_code, content=error_reponse.model_dump())
...
async def handle_application_exceptions(request: Request, exc: ApplicationException):
    ...
    error_reponse = ApiResponse.failure(
        ErrorResponse(code=exc.error_code, type="Application", details=exc.details),
        exc.message,
    )
    return JSONResponse(status_code=exc.status_code, content=error_reponse.model_dump())
...
async def handle_pydantic_validation_errors(request: Request, exc: ValidationError):
    ...
    error_resonse = ApiResponse.failure(
        ErrorResponse(
            code="DATA_FORMAT_ERROR", type="Data Validation", details=errors_details
        ),
        "One or more validation errors occurred in the request data.",
    )
    return JSONResponse(
        status_code=HTTPStatus.UNPROCESSABLE_ENTITY, content=error_resonse.model_dump()
    )
...
async def handle_value_errors(request: Request, exc: ValueError):
    ...
    error_response = ApiResponse.failure(
        ErrorResponse(code="INVALID_INPUT", type="Value Error", details={}), str(exc)
    )
    return JSONResponse(
        status_code=HTTPStatus.BAD_REQUEST, content=error_response.model_dump()
    )
