...
class GetProductByIdUseCase:
    ...
    def execute(self, query: GetProductByIdQuery) -> ProductDetails:

        product = self.product_repository.get_by_id(query.product_id)

        if not product:
            raise ProductNotFoundError("Product", query.product_id.to_string())

        print(f"Product found: {product.name} with ID {product.id}")
...
class SearchProductUseCase:
    ...
    def execute(self, product_params: SearchProductsQuery) -> List[ProductDetails]:
        product_list = self.product_repository.search(product_params)
...
class CreateProductUseCase:
    ...
    def execute(self, create_data: ProductCreateCommand) -> ProductDetails:
        self._validate_category(create_data)

        new_product = Product(**create_data.model_dump())
        product_created = self.product_repository.save(new_product)
...
class UpdateProductUseCase:
    ...
    def execute(self, update_data: ProductUpdateCommand) -> ProductDetails:
        product = self.product_repository.get_by_id(update_data.product_id)
        if not product:
            raise ProductNotFoundError("Product", update_data.product_id)
