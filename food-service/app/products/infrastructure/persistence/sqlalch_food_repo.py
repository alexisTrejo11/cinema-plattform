from app.products.application.repositories import FoodCategoryRepository, FoodRepository
from app.products.domain.entities import FoodProduct
from typing import Dict, Optional, List
from sqlalchemy.orm import Session
from .models import FoodProductModel
from app.products.application.dtos import SearchFoodParams
...
class SqlAlchFoodRepository(FoodRepository):
    ...
    def get_by_id_in(self, product_id_list: List[int]) -> Dict[int, FoodProduct]:
        if not product_id_list:
            return {}
        
        models = self.session.query(FoodProductModel).filter(
            FoodProductModel.id.in_(product_id_list),
            FoodProductModel.is_available == True
        ).all()
        
        return {model.id: model.to_domain() for model in models}
