...
product_router = APIRouter(prefix="/products", tags=["Food Products"])

@product_router.post("/", response_model=FoodProductResponse, status_code=201)
def create_product(
...
@product_router.get("/{product_id}", response_model=FoodProductResponse)
def search_product(
    product_id: int,
...
@product_router.get("/", response_model=List[FoodProductResponse])
def search_products(
...
@product_router.patch("/{product_id}", response_model=FoodProductResponse)
def update_product(
    product_id: int,
