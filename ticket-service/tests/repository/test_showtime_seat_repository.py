...
class TestSqlAlchemySeatRepository:
    async def test_list_by_id_in(self, seat_repository, multiple_seat_models):
        ...
        assert len(seats) == 3
        assert all(seat.get_id() in id_list for seat in seats)
    ...
    async def test_list_by_showtime(self, seat_repository, multiple_seat_models):
        ...
        seats: List[ShowtimeSeat] = await seat_repository.list_by_showtime(showtime_id)
        
        assert len(seats) == len(multiple_seat_models)
        assert all(seat.get_showtime_id() == showtime_id for seat in seats)
        assert all(isinstance(seat, ShowtimeSeat) for seat in seats)
    ...
    async def test_save_new_seat(self, seat_repository, sample_seat_entity, async_session):
        ...
        # For new seats, the ID should be assigned after saving
        assert saved_seat.get_id() is not None
        assert sample_seat_entity.get_id() is None  # Original entity should have None ID
        
        assert saved_seat.get_showtime_id() == sample_seat_entity.get_showtime_id()
        assert saved_seat.get_seat_id() == sample_seat_entity.get_seat_id()
        assert saved_seat.get_seat_name() == sample_seat_entity.get_seat_name()
        assert saved_seat.get_is_available() == sample_seat_entity.get_is_available()
        assert saved_seat.get_ticket_id() == sample_seat_entity.get_ticket_id()
        
        # Verify in database
        stmt = select(ShowtimeSeatModel).where(ShowtimeSeatModel.seat_id == saved_seat.get_seat_id())
    ...
    async def test_save_existing_seat(self, seat_repository, sample_seat_model, async_session):
        ...
        existing_seat = ShowtimeSeat(
            ...
            is_available=False,  # Changing availability
            ticket_id=1,  # Adding ticket_id
            created_at=sample_seat_model.created_at
        )
        
        updated_seat: ShowtimeSeat = await seat_repository.save(existing_seat)
        
        assert updated_seat.get_seat_id() == sample_seat_model.seat_id
        assert updated_seat.get_is_available() is False
        assert updated_seat.get_ticket_id() == 1
        
        # Verify in database
        stmt = select(ShowtimeSeatModel).where(ShowtimeSeatModel.seat_id == sample_seat_model.seat_id)
        result = await async_session.execute(stmt)
        db_seat = result.scalar_one()
    ...
    async def test_bulk_update(self, seat_repository, multiple_seat_models, async_session):
        ...
        stmt = select(ShowtimeSeatModel).where(ShowtimeSeatModel.id.in_([s.get_id() for s in seats_to_update]))
        result = await async_session.execute(stmt)
