...
async def search_tickets(
    ...
    created_before: Optional[datetime] = Query(
        None, description="Filter tickets created before this date"
    ),
    created_after: Optional[datetime] = Query(
        None, description="Filter tickets created after this date"
    ),
    ...
    sort_direction_asc: bool = Query(
        True, description="Sort direction (True for ascending)"
    ),
...
async def list_user_tickets(
    user_id: int = Path(
        ..., description="ID of the user", json_schema_extra={"example": 456}
    ),
...
async def list_showtime_tickets(
    showtime_id: int = Path(
        ..., description="ID of the showtime", json_schema_extra={"example": 789}
    ),
    include_seats: bool = Query(
        True,
        description="Include seat information",
