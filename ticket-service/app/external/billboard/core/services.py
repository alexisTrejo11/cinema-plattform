...
from app.external.billboard.core.interfaces import (
    CinemaRepository,
    TheaterRepository,
    ShowtimeRepository,
)
...
from app.external.billboard.core.entities.theater_seat import TheaterSeat
...
class TheaterSeatService:
    ...
    async def get_seats_by_ids(
        self, cinema_id: int, theater_id: int, seat_id: int
    ) -> Optional[TheaterSeat]:
    ...
    async def save_seat_list(
        self, cinema_id: int, theater_id: int, seats: List[TheaterSeat]
    ) -> None:
        theater = await self.theater_service.get_by_id(cinema_id, theater_id)
        if not theater:
