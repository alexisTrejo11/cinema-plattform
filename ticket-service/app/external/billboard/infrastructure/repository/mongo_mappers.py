from app.external.billboard.core.enums.theather_enum import TheaterType, SeatType
from pymongo.database import Database
from app.external.billboard.core.entities.theater import TheaterSeat, Theater
...
class CinemaMongoMapper:
    ...
    def doc_to_domain(data: Dict[str, Any]) -> Cinema:
        theaters_doc = data.get("theaters") or data.get("theathers") or []
    ...
    def domain_to_doc(cinema: Cinema) -> Dict[str, Any]:
        doc = {
            ...
            "theaters": [TheaterMongoMapper.domain_to_doc(t) for t in cinema.theaters],
...
class TheaterMongoMapper:
    ...
    def doc_to_domain(data: Dict[str, Any]) -> Theater:
        ...
        theater_id = data.get("theater_id") or data.get("id")
        if theater_id is None:
            raise ValueError("theater document missing theater_id")
...
class ShowtimeDocMapper:
    ...
    def doc_to_domain(data: Dict[str, Any]) -> Showtime:
        sid = data.get("showtime_id") or data.get("id")
        if sid is None:
            raise ValueError("showtime document missing id/showtime_id")
        return Showtime(
            id=sid,
            movie=data["movie"],
            cinema=data["cinema"],
    ...
    def domain_to_doc(showtime: Showtime) -> Dict[str, Any]:
        doc = dict(showtime.to_dict())
        doc["showtime_id"] = showtime.id
        doc["id"] = showtime.id
        return doc
