...
@router.get("/cinemas/")
async def list_cinemas(cinema_repo: MongoCinemaRepository = Depends(get_cinema_repo)):
    """
...
async def get_cinema_by_id(
    cinema_id: int, cinema_repo: MongoCinemaRepository = Depends(get_cinema_repo)
):
    """
    Obtiene un teatro por su ID de negocio (int).
    """
