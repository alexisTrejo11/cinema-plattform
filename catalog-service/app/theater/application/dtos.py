...
from app.theater.domain.enums import TheaterType
...
class SearchTheaterFilters(BaseModel):
    ...
    cinema_id: Optional[int] = Field(
        None, description="Filter by cinema ID.", examples=[1]
    )
...
class TheaterSummaryResponse(BaseModel):
    ...
    name: str = Field(
        ..., description="Theater display name.", examples=["IMAX Room 1"]
    )
    capacity: int = Field(..., ge=1, description="Seat capacity.", examples=[180])
    theater_type: TheaterType = Field(
        ..., description="Theater type.", examples=[TheaterType.IMAX]
    )
    is_active: bool = Field(
        ..., description="Whether theater is active.", examples=[True]
    )
...
class PaginatedTheaterResponse(PaginationMetadata):
