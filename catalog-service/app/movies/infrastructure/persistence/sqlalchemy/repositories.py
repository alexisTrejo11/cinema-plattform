...
class SQLAlchemyMovieRepository(MovieRepository):
    ...
    def _build_filters(self, filters: SearchMovieFilters) -> List[Any]:
        ...
        if filters.genre:
            conditions.append(MovieModel.genre == filters.genre)
        if filters.rating:
            conditions.append(MovieModel.rating == filters.rating)
        if filters.is_active is not None:
            conditions.append(MovieModel.is_active == filters.is_active)
        if filters.release_date_from:
            conditions.append(MovieModel.release_date >= filters.release_date_from)
        if filters.release_date_to:
            conditions.append(MovieModel.release_date <= filters.release_date_to)
        if filters.min_duration is not None:
            conditions.append(MovieModel.minute_duration >= filters.min_duration)
        if filters.max_duration is not None:
            conditions.append(MovieModel.minute_duration <= filters.max_duration)
