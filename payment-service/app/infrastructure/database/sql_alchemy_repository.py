...
from app.payment.domain.repository.payment_repository import PaymentRepository
from .models import PaymentModel, ModelMapper
...
class SqlAlchemyPaymentRepository(PaymentRepository):
    ...
    async def soft_delete(self, payment_id: PaymentId) -> bool:
        stmt = select(PaymentModel).where(
            PaymentModel.id == payment_id.__str__, 
            PaymentModel.deleted_at != None
        )
        result = await self.session.execute(stmt)
        model = result.scalar_one_or_none()
