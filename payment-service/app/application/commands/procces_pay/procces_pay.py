...
from app.domain.value_objects import UserId, Money, PaymentMethod, PaymentType, PaymentMetadata, Currency
...
from .command import ProcessPayCommand
from .result import ProcessPaymentResult
...
class ProcessPaymentCommandHandler:
    ...
    async def handle(self, command: ProcessPayCommand) -> ProcessPaymentResult:
        ...
        try:
            ...
            payment = Payment.create(user_id, amount, payment_method, payment_type, payment_metadata)
    ...
    async def _process_wallet_payment(self, payment: Payment, wallet_id: Optional[UUID]) -> Dict[str, Any]:
        """Process payment using wallet funds."""
        wallet = await self._get_wallet_by_id(wallet_id) if wallet_id else await self._get_user_wallet(payment.user_id)
        
        if not wallet.can_debit(payment.amount):
            raise InsufficientFundsException(wallet.get_available_balance().to_float(), payment.amount.to_float())
        
        transaction_id = wallet.debit(payment.amount, f"Payment for {payment.payment_type.value}", str(payment.id))
        ...
        return {'transaction_id': str(transaction_id), 'wallet_id': str(wallet.id), 'method': 'wallet'}
    
    async def _process_external_payment(self, payment: Payment, command: ProcessPayCommand) -> Dict[str, Any]:
    ...
    async def _send_notifications(self, payment: Payment, processing_result: Dict[str, Any]) -> None:
    ...
    async def _proccess_payment_failure(self, payment: Optional[Payment], message: str) -> ProcessPaymentResult:
    ...
    async def _get_wallet_by_id(self, wallet_id: UUID) -> Wallet:
        wallet = await self.wallet_repository.get_by_id(WalletId.from_string(str(wallet_id)))
