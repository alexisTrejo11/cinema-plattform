...
class Wallet(BaseModel):
    ...
    MIN_CREDIT_AMOUNT: ClassVar[float] = 0.01
    MIN_DEBIT_AMOUNT: ClassVar[float] = 0.01
    MAX_BALANCE_LIMIT: ClassVar[float] = 10000.00
    
    @model_validator(mode="after")
    def init_and_validate(self) -> Wallet:
        """Initialize derived fields and validate business rules."""
        if self.minimum_balance is None:
            self.minimum_balance = Money.zero(self.currency)
