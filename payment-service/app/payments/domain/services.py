...
class PaymentDomainService:
    ...
    def calculate_payment_fees(
        self, amount: Money, payment_method: PaymentMethod, payment_type: PaymentType
    ) -> Money:
        """
        ...
        return Money(amount=fee_amount, currency=amount.currency)

    def calculate_bulk_discount(
        self, base_amount: Money, item_count: int, payment_type: PaymentType
    ) -> Money:
        """
        ...
        if item_count >= self.BULK_PAYMENT_DISCOUNT_THRESHOLD:
            ...
            return Money(amount=discount_amount, currency=base_amount.currency)

        return Money.zero(base_amount.currency)

    def calculate_refund_amount(
        self, payment: Payment, refund_percentage: Optional[Decimal] = None
    ) -> Money:
        """
        ...
        if not payment.can_be_refunded():
            raise PaymentNotRefundableException(
                str(payment.id), "Payment is not in a refundable state"
            )
    ...
    def calculate_payment_expiry(
        self, payment_type: PaymentType, payment_method: PaymentMethod
    ) -> datetime:
        """
    ...
    def _apply_ticket_refund_policy(
        self, payment: Payment, base_refund: Money
    ) -> Money:
        """
