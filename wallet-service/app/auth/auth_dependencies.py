...
def get_token_service() -> JWTTokenService:
    ...
    """
    return JWTTokenService(secret_key=settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
...
def get_employee_user(logged_user: User = Depends(role_required([UserRole.MANAGER, UserRole.ADMIN, UserRole.EMPLOYEE]))) -> User:
    return logged_user

def get_customer_user(logged_user: User = Depends(role_required([UserRole.MANAGER, UserRole.CUSTOMER, UserRole.ADMIN]))) -> User:
    return logged_user