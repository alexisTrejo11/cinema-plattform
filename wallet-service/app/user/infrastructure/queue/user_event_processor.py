...
from app.user.application.usecases import CreateUserUseCase, UpdateUserUseCase, SoftDeleteUserUseCase, UserCreateCommand
from app.user.domain.repository import UserRepository
from app.user.infrastructure.sql_user_respository import SqlAlchemyUserRepository
from config.postgres_config import get_db as get_db_session
...
class UserEventProcessor:
        ...
        async for session in get_db_session():
            ...
            try:
                match event_type:
                    ...
                    case "user.updated":
                        await self._proccess_user_update_event(user_data, user_repo, payload)
                    case "user.deleted":
                        await self._proccess_user_delete_event(user_data, user_repo, payload)
                    case _:
    ...
    async def _proccess_user_update_event(self, user_data: Dict[str, Any], user_repo, payload):
    ...
    async def _proccess_user_delete_event(self, user_data: Dict[str, Any], user_repo, payload):
        user_id_str = str(user_data.get("id"))
        if user_id_str:
