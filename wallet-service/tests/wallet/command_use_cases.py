...
from app.wallet.domain.value_objects import WalletId, Money, Charge
...
def dummy_transaction_domain(dummy_wallet_id: WalletId) -> WalletTransaction:
    ...
    transaction.id = WalletId(
        uuid.uuid4()
    )  # Assuming TransactionId is WalletId for now
    transaction.wallet_id = dummy_wallet_id
    transaction.amount = Money(Decimal("10.00"), Currency.USD)
...
def dummy_wallet_domain(
    ...
    wallet.balance = Money(Decimal("100.00"), Currency.USD)
...
class TestPayWithWalletUseCase:
    ...
    def pay_with_wallet_command(
        ...
        return PayWithWalletCommand(
            wallet_id=dummy_wallet_id,
            charge=Charge(Decimal("25.00"), Currency.USD),
            payment_details=dummy_payment_details,
        )
