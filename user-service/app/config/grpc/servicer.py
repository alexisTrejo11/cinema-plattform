...
class UsersGrpcServicer(users_pb2_grpc.UsersServiceServicer):
    async def GetUserById(
        self, request: users_pb2.GetUserByIdRequest, context: aio.ServicerContext
    ):
        if request.id <= 0:
            await context.abort(
                grpc.StatusCode.INVALID_ARGUMENT, "id must be a positive integer"
            )
    ...
    async def GetUserByEmail(
        self, request: users_pb2.GetUserByEmailRequest, context: aio.ServicerContext
    ):
    ...
    async def GetUserByPhone(
        self, request: users_pb2.GetUserByPhoneRequest, context: aio.ServicerContext
    ):
