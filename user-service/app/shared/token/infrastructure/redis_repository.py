...
from app.shared.token.core import Token, TokenType, TokenRepository
import logging
...
def json_serializer(obj):
    ...
    if isinstance(obj, Enum):
        return obj.value
    logger.debug(
        f"json_serializer encountered unhandled type: {type(obj)} with value: {obj}"
    )
...
class RedisTokenRepository(TokenRepository):
        ...
        try:
            ...
            if token_type_str:
                try:
                    ...
                    logger.warning(
                        f"Invalid TokenType value '{token_type_str}' in session data for user_id: {session_data.get('user_id')}. Returning None."
                    )
                    return None
    ...
    def create(self, token: Token) -> None:
        ...
        if expires_in_seconds > 0:
            ...
            logger.warning(
                f"Token for user {token.user_id} expires in the past or is non-expiring. Not setting expiration."
            )
    ...
    def revoke_user_token(
        ...
        try:
            ...
            self.redis_conn.set(
                key, json.dumps(session_data, default=json_serializer)
            )  # Synchronous call
            return True
        except (json.JSONDecodeError, KeyError, AttributeError) as e:
            ...
            if result:
                try:
                    ...
                    if not session_data.get("is_revoked", False):
                        ...
                        update_pipe.set(
                            all_keys[i],
                            json.dumps(session_data, default=json_serializer),
                        )
                        updated_count += 1
                except (json.JSONDecodeError, KeyError, AttributeError):
                    continue
    ...
    def cleanup_expired_tokens(self) -> int:
        ...
        while True:
            cursor, keys = self.redis_conn.scan(  # Synchronous scan
                cursor=cursor, match="session:*:*:*", count=1000
            )
            ...
            for i, result in enumerate(results):
                if result:
                    try:
                            ...
                            if expires_at_str:
                                ...
                                if expires_at < current_time:
                                    ...
                                    update_pipe.set(
                                        keys[i],
                                        json.dumps(
                                            session_data, default=json_serializer
                                        ),
                                    )
                                    count += 1
                    except (
                        ...
                        logger.error(
                            f"Error processing session data for cleanup for key {keys[i]}: {e}. Data: {result}"
                        )
                        continue
