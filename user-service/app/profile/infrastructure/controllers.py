...
@router.get(
    ...
    responses={
        200: {
            "model": ApiResponse[ProfileResponse],
...
async def get_my_profile(
    ...
    """
    logger.info(
        f"GET profile started | user_id:{user.id} | client:{request.client.host if request.client else None}"
    )
    try:
...
async def update_my_profile(
    ...
    """
    logger.info(
        f"PATCH profile started | user_id:{user.id} | client:{request.client.host if request.client else None}"
    )
    try:
